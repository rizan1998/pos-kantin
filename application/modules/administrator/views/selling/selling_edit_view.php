...
<?php $this->load->view('parts/main_part'); ?>
    ...
    <div class="row clearfix">
            ...
            <div class="card planned_task">
                <div class="header">
                    <div class="row">
                        ...
                        <div class="col-lg-6">
                            <form action="<?php echo base_url()?>selling-edit/<?php echo $selling['id']?>" method="get">
                                <div class="input-group">
                                    ...
                                    <div class="input-group-append">
                                        ...
                                        <a href="<?php echo base_url()?>selling-edit/<?php echo $selling['id']?>" type="button" class="btn btn-primary"><i class="fa fa-refresh"></i></a>
                                    </div>
                                </div>
                            </form>
                        </div>
                        ...
                        <?php 
                            ...
                            foreach ($category as $cat){
                                ...
                                if(isset($_GET['cari'])){
                                    $cari = $_GET['cari'];
                                        if(!empty($cari)){
                                            $sql = "SELECT `isell`.`id`, `isell`.`inc_id`, isell.item_id, `i`.`code`, `i`.`name`, `u`.`inc_id` as `unit_id`, `u`.`name` as `unit_name`, `discount`, `price_sell`, `type_price` FROM `items_sell` `isell` LEFT JOIN `items` `i` ON `isell`.`item_id` = `i`.`inc_id` LEFT JOIN `unit` `u` ON `i`.`unit_id` = `u`.`inc_id` WHERE `i`.`category_id` = ".$cat['inc_id']." AND `isell`.`type_price` = 1 AND (i.name like '%$cari%' or i.code like '%$cari%') ";
                                        }else {
                                            $sql = "SELECT `isell`.`id`, `isell`.`inc_id`, isell.item_id, `i`.`code`, `i`.`name`, `u`.`inc_id` as `unit_id`, `u`.`name` as `unit_name`, `discount`, `price_sell`, `type_price` FROM `items_sell` `isell` LEFT JOIN `items` `i` ON `isell`.`item_id` = `i`.`inc_id` LEFT JOIN `unit` `u` ON `i`.`unit_id` = `u`.`inc_id` WHERE `i`.`category_id` = ".$cat['inc_id']." AND `isell`.`type_price` = 1";
                                        }
                                }else {
                                        $sql = "SELECT `isell`.`id`, `isell`.`inc_id`, isell.item_id, `i`.`code`, `i`.`name`, `u`.`inc_id` as `unit_id`, `u`.`name` as `unit_name`, `discount`, `price_sell`, `type_price` FROM `items_sell` `isell` LEFT JOIN `items` `i` ON `isell`.`item_id` = `i`.`inc_id` LEFT JOIN `unit` `u` ON `i`.`unit_id` = `u`.`inc_id` WHERE `i`.`category_id` = ".$cat['inc_id']." AND `isell`.`type_price` = 1";
                                }
                                ...
                                foreach ($query->result_array() as $i){
                                        ...
                                        $tab_content .= '
                                                <div class="col-lg-4 col-md-12">
                                                    <a data-toggle="modal" href="#modal-'.$i['inc_id'].'">
                                                        <div class="card">
                                                            <div class="body">
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <div class="">
                                                                    <label>Qty</label>
                                                                    <input type="number" class="form-control" value="1" name="qty" min="0" id="qty-'.$i['inc_id'].'"  autofocus>
        ...
        <div class="col-lg-4 col-md-12">
            <div class="card planned_task">
                <div class="header">
                    <h2>NOTA :  <?php echo $selling['nota'] ?></h2>
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                      ...
                      <input type="text"
                        class="form-control" name="employee" id="employee" placeholder="pastikan isi nama lengkap">
                    </div>
                    <div class="form-group">
                      <label for="">Total Hutang</label>
                      <input type="text"
                        class="form-control" name="total_hutang" id="total_hutang" readonly>
                    </div>
                </div>
                <div class="modal-footer">
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            if(req.info=="yes"){		
                                ...
                                setTimeout(
                                    window.open("<?php echo base_url()?>administrator/selling/print_bill/"+req.id, "_blank")
                                    , 4000);
                                setTimeout(location.href="<? echo base_url() ?>selling", 6000)
                            }
                        },
                        error:function(){
                            swal.fire("Error!", "data gagal tersimpan", "error");
                        ...
                        $.ajax({
                            ...
                            success: function(data){
                                ...
                                if(req.info=="yes"){		
                                    ...
                                    setTimeout(
                                        location.href = "<?php echo base_url()?>selling"
                                        , 20000);
                                }
                            },
                            error:function(){
