                ...
                <div class="body">
                    <form id="basic-form" method="post" novalidate>
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control clear" required name="fullname" id="fullname">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control clear" required name="username" id="username">
                        </div>
                        <div class="form-group">
                            <label>Password</label>
                            <input type="password" class="form-control clear" required name="password" id="password">
                        </div>
    ...
    <script>
        ...
        $(document).ready(function(){
            table=$('#data-datatable').DataTable({
                ...
                "order": [], 
                "ajax": {
                    "url": "<?php echo base_url(); ?>administrator/user/ajx_get_data/",
                    "type": "POST"
                },
            ...
            $("#basic-form").submit(function(){
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url(); ?>administrator/user/create_process",
                    data: data_val,
            ...
            $('#data-datatable').on('click', '#edit', function () {
                ...
                $.get("<?php echo base_url(); ?>administrator/user/get_where_user/"+id, function(data){
                    var items=JSON.parse(data);
                    $("#fullname").val(items.fullname);
                    $("#username").val(items.username);
                    $("#roles").val(items.roles);
            ...
            $('#data-datatable').on('click', '#delete', function () {
                ...
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.get("<?php echo base_url(); ?>administrator/user/user_delete/"+id, function(data){
                            var req=JSON.parse(data);
                            if(req.info=="yes"){
                                table.ajax.reload(); 
